...
def compute_average_alignment_percentage_of_identity_parallel(alignment: Alignment, config_args: Config, executor):
    ...
    future_results = [executor.submit(__compute_average_percentage_of_identity,
                                      sequences, processor_pairs, config_args) for processor_pairs in processors_jobs]
...
def filter_alignments_parallel(alignments_file: str, min_alignment_length: int, min_alignment_coverage: int, min_alignment_identity: float, ingroup_size: int, config_args: Config):
    ...
    for alignment in alignments:
        ...
        if alignment_length >= min_alignment_length:
            ...
            if alignment_presence >= min_alignment_coverage:
                ...
                alignment_percentage_identity = compute_average_alignment_percentage_of_identity_parallel(
                ...
                if alignment_percentage_identity >= min_alignment_identity:
                    ...
                    d["sequences"] = get_alignment_sequences(
                        alignment=alignment, percentage_identity=alignment_percentage_identity)
    ...
    logging.info("Percentage kept: {}".format(
...
def run_filter(config_args: Config):
    ...
    success = save_alignments(alignments=filtered_alignments_reduced, config_args=config_args,
