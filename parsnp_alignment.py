...
def parsnp_wrapper(input_directory: str, reference_genome_with_path: str,  output_directory: str, number_threads: int):
    ...
    try:
        ...
        command = "parsnp  -p" + str(number_threads) + " -r " + reference_genome_with_path + \
            " -d " + input_directory + " -o " + output_directory + " -c"
...
def verify_ingroup_folder(config_args: Config):
    ...
    genome_filepaths = get_files_in_dir_recursive(
        folder_path=config_args.ingroup_path)
...
def run_parsnp(config_args: Config, reference_genome: str = None):
    ...
    if len(xmfa_filepath) > 0:
        ...
        xmfa_filelocation = os.path.join(
            xmfa_filepath, config_args.xmfa_file_name)
        if output == 0 or os.path.isfile(xmfa_filelocation):
            ...
            logging.info("xmfa location: {}".format(
                config_args.xmfa_file_path))
        else:
            logging.error("Something went wrong with Parsnp {}".format(output))
