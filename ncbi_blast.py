...
def ncbi_blast_bio(
    ...
    try:
        if max_hsps == None:
            output = NcbiblastnCommandline(query=query_file, subject=subject_file, evalue=e_cutoff,
                                           perc_identity=identity_perc_cutoff, outfmt=5, task="blastn-short")()[0]
        else:
            output = NcbiblastnCommandline(query=query_file, subject=subject_file, evalue=e_cutoff,
                                           perc_identity=identity_perc_cutoff, max_hsps=max_hsps, outfmt=5, task="blastn-short")()[0]
        print(output)
...
def ncbi_blast_wrapper_cl(query_file: str, subject_file: str,  e_cutoff: float, identity_perc_cutoff: float, max_hsps: int):
    ...
    try:
        if max_hsps == None:
            command = "blastn -outfmt 5 -query " + query_file + " -evalue " + \
                str(e_cutoff) + " -subject " + subject_file + " -task=blastn-short " + \
                " -perc_identity " + str(identity_perc_cutoff)
        else:
            command = "blastn -outfmt 5 -query " + query_file + " -evalue " + \
                str(e_cutoff) + " -subject " + subject_file + " -task=blastn-short " + \
                " -perc_identity " + \
                str(identity_perc_cutoff) + " -max_hsps " + str(max_hsps)
        ...
        if processor_index < config_args.processors_number and subject_index < total_subjects:
            ...
            if processor_index >= config_args.processors_number or subject_index >= total_subjects:
                ...
                future_results = [executor.submit(
                    blast_multifasta_files_vs_subject, subject_info, query_fasta_paths, config_args) for subject_info in processors_tasks]
...
def blast_subsequences_against_genomes(genomes_query: List[Genome],
    ...
    for query_index in range(0, len(genomes_query)):
        ...
        blast_results = blast_query_subsequences_vs_outgroup(
            config_args=config_args, query_fasta_paths=query_fasta_paths, genomes_subject=genomes_subject, subject_sequence_fasta_paths=subject_sequence_fasta_paths)
...
def run_blast():
    ...
    ncbi_blast_bio(query_file=query_file, subject_file=subject_file,
                   e_cutoff=1E-40, identity_perc_cutoff=90, max_hsps=1)
