...
import logging
import time
from sequence_finder import get_unique_seq_from_alignments
...
def main():
    ...
    if success:
        alignment_genomes = get_unique_seq_from_alignments(
            config_args=config_args)
        end = time.time()
        mins = (end-start)/60
        config_args.stats.end_time = end
        config_args.stats.total_runtime = mins
        create_general_report(config_args=config_args,
                              unique_sequences=alignment_genomes, mins=mins)
