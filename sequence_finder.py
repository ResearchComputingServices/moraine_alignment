...
def write_subsequences_to_files(genomes: list, file_location: str, number_processors: int) -> list:
    ...
    for genome in genomes:
        logging.info(
            "Genome {} -> # Sequences to write: {}".format(genome.id, len(genome.subsequences)))
...
def get_alignment_subsequences(config_args: Config, alignment: dict) -> list:
    ...
    subsequence_files = write_subsequences_to_files_fixed_files(
        genomes=[alignment], file_location=config_args.in_process_path, number_processors=config_args.processors_number)
...
def get_unique_seq_from_alignments(config_args: Config):
    ...
    # outgroup_genomes = read_genomes_duplicate(
    #    config_args=config_args, larger_size=500, type="Outgroup")
    ...
    if config_args.filtered_xmfa_path != "":
        ...
        if success:
            for filtered_alignment in filtered_alignments.items():
                logging.info("\n")
                logging.info("Procesing Alignment {} of {} --------------------------------".format(
                ...
                total_subsequences_from_alignments = total_subsequences_from_alignments + \
