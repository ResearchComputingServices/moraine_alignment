class Stats:
    ...
    def set_alignments_discarded_by_length(self, count):
        ...
        if self.alignments_found_by_parsnp > 0:
            self.percentage_discarded_by_length = self.alignments_discarded_by_length / \
    ...
    def set_alignments_discarded_by_coverage(self, count):
        ...
        if self.alignments_found_by_parsnp > 0:
            self.percentage_discarded_by_coverage = self.alignments_discarded_by_coverage / \
                self.alignments_found_by_parsnp

    def set_alignments_discared_by_percentage_of_identity(self, count):
        self.alignments_discared_by_percentage_of_identity = count
        if self.alignments_found_by_parsnp > 0:
            self.percentage_discared_by_percentage_of_identity = self.alignments_discared_by_percentage_of_identity / \
                self.alignments_found_by_parsnp

    def compute_total_alignnments_kept(self):
        self.total_alignments_taken = self.alignments_discarded_by_length + \
        ...
        if self.alignments_found_by_parsnp > 0:
            self.percentage_alignment_taken = self.total_alignments_taken / \
                self.alignments_found_by_parsnp
            self.percentage_alignment_kept = self.total_alignments_kept / \
                self.alignments_found_by_parsnp
